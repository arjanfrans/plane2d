...
#if ELPP_COMPILER_GCC || ELPP_COMPILER_CLANG || ELPP_COMPILER_INTEL ||                                  \
    (ELPP_COMPILER_MSVC && _MSC_VER >= 1800)
#define ELPP_VARIADIC_TEMPLATES_SUPPORTED 1
#endif // ELPP_COMPILER_GCC || ELPP_COMPILER_CLANG || ELPP_COMPILER_INTEL || (ELPP_COMPILER_MSVC &&
       // _MSC_VER >= 1800)
...
#endif // (!defined(ELPP_DISABLE_DEBUG_LOGS) && (ELPP_LOGGING_ENABLED) && ((defined(_DEBUG)) ||
       // (!defined(NDEBUG))))
...
enum class Level : base::type::EnumType {
    /// @brief Generic level that represents all the levels. Useful when setting global configuration for
    /// all levels
    Global = 1,
    /// @brief Information that can be useful to back-trace certain events - mostly useful than debug
    /// logs.
    Trace = 2,
...
public:
    ...
    /// @param fn function to apply with each level. This bool represent whether or not to stop iterating
    /// through levels.
    static inline void forEachLevel(base::type::EnumType *startIndex,
...
enum class ConfigurationType : base::type::EnumType {
    ...
    ///
    /// @detail This does not depend on logger or level. Performance tracking always uses 'performance'
    /// logger
    PerformanceTracking = 64,
    /// @brief Specifies log file max size.
    ///
    /// @detail If file size of corresponding log file (for corresponding level) is >= specified size,
    /// log file will
...
public:
    ...
    /// @brief Represents maximum valid configuration type. This is used internally and you should not
    /// need it.
    ...
    /// @param startIndex initial value to start the iteration from. This is passed by pointer and is
    /// left-shifted
...
} kTimeFormats[] = {{1000.0f, ELPP_LITERAL("mis")},
...
/// @brief Bitwise operations for C++11 strong enum class. This casts e into Flag_T and returns value
...
public:
    ...
    static base::type::fstream_t *newFileStream(const std::string &filename) {
        base::type::fstream_t *fs = new base::type::fstream_t(
            filename.c_str(), base::type::fstream_t::out | base::type::fstream_t::app);
#if defined(ELPP_UNICODE)
...
public:
    ...
    /// @brief Replaces all instances of replaceWhat with 'replaceWith'. Original variable is changed for
    /// performance.
    ...
    /// @brief Replaces all instances of 'replaceWhat' with 'replaceWith'. (String version) Replaces in
    /// place
    ...
    static void replaceFirstWithEscape(base::type::string_t &str,
                                       const base::type::string_t &replaceWhat, // NOLINT
                                       const std::string &replaceWith) {
        replaceFirstWithEscape(str, replaceWhat,
...
public:
    ...
    /// @param alternativeBashCommand If environment variable not found what would be alternative bash
    /// command
    ///        in order to look for value user is looking for. E.g, for 'user' alternative command will
    ///        'whoami'
...
public:
    /// @brief Cross platform gettimeofday for Windows and unix platform. This can be used to determine
    /// current millisecond.
    ///
    /// @detail For unix system it uses gettimeofday(timeval*, timezone*) and for Windows, a seperate
    ...
    /// @brief Formats time to get unit accordingly, units like second if > 1000 or minutes if > 60000
    /// etc
        ...
        for (; *format; ++format) {
            if (*format == base::consts::kFormatSpecifierChar) {
                ...
                case 'y': // year (two digits)
                    buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_year + base::consts::kYearBase,
                                                                2, buf, bufLim);
                    continue;
...
public:
    ...
    inline friend base::type::ostream_t &operator<<(base::type::ostream_t &os,
...
///
/// @detail Most of the functions are virtual final methods but anything implementing this abstract class
/// should implement
/// unregisterAll() and deepCopy(const AbstractRegistry<T_Ptr, Container>&) and write registerNew()
/// method according to container
/// and few more methods; get() to find element, unregister() to unregister single entry.
/// Please note that this is thread-unsafe and should also implement thread-safety mechanisms in
/// implementation.
...
///
/// @detail NOTE: This is thread-unsafe implementation (although it contains lock function, it does not
/// use these functions)
...
public:
    ...
    /// @brief Copy constructor that is useful for base classes. Try to avoid this constructor, use move
    /// constructor.
    ...
    /// @brief Assignment operator that unregisters all the existing registeries and deeply copies each
    /// of repo element
    /// @see unregisterAll()
...
/// @brief A pointer registry mechanism to manage memory and provide search functionalities. (predicate
/// version)
///
...
public:
    ...
    friend inline base::type::ostream_t &operator<<(base::type::ostream_t &os,
...
protected:
    ...
    template <typename T, typename T2> inline T_Ptr *get(const T &arg1, const T2 arg2) {
...
public:
    LogFormat(void)
        : m_level(Level::Unknown), m_userFormat(base::type::string_t()),
          m_format(base::type::string_t()), m_dateTimeFormat(std::string()), m_flags(0x0) {
    }

    LogFormat(Level level, const base::type::string_t &format) : m_level(level), m_userFormat(format) {
...
protected:
    ...
    /// @brief Updates %level from format. This is so that we dont have to do it at log-writing-time. It
    /// uses m_format and m_level
    virtual void updateFormatSpec(void) ELPP_FINAL {
        // Do not use switch over strongly typed enums because Intel C++ compilers dont support them yet.
        if (m_level == Level::Debug) {
            base::utils::Str::replaceFirstWithEscape(m_format,
                                                     base::consts::kSeverityLevelFormatSpecifier,
            ...
            base::utils::Str::replaceFirstWithEscape(
                m_format, base::consts::kSeverityLevelFormatSpecifier, base::consts::kInfoLevelLogValue);
            base::utils::Str::replaceFirstWithEscape(m_format,
                                                     base::consts::kSeverityLevelShortFormatSpecifier,
...
/// @brief Represents single configuration that has representing level, configuration type and a string
/// based value.
///
/// @detail String based value means any value either its boolean, integer or string itself, it will be
...
///   * el::Configuration confMaxLogFileSizeInfo(el::Level::Info, el::ConfigurationType::MaxLogFileSize,
///   "2048");
...
public:
    ...
    /// @param value Value to set. Values have to be std::string; For boolean values use "true", "false",
    /// for any integral values
...
///
/// @detail This repository represents configurations for all the levels and configuration type mapped to
/// a value.
...
public:
    ...
    /// @param configurationFile Full path to configuration file
    /// @param base Configurations to base new configuration repository off. This value is used when you
    /// want to use
    ///        existing Configurations to base all the values and then set rest of configuration via
    ///        configuration file.
    /// @return True if successfully parsed, false otherwise. You may define 'ELPP_DEBUG_ASSERT_FAILURE'
    /// to make sure you
    ...
    ///
    /// @detail This configuration string has same syntax as configuration file contents. Make sure all
    /// the necessary
    ...
    /// configuration types
    /// ConfigurationType::MillisecondsWidth and ConfigurationType::PerformanceTracking will be ignored
    /// if not set for
    ...
    /// @param value A string based value. Regardless of what the data type of configuration is, it will
    /// always be string
    ...
    /// @see Configurations::set(Level level, ConfigurationType configurationType, const std::string&
    /// value)
    ...
    /// configuration file that sets
    /// configuration for all the configurations except for Enabled or not, we use this so that ENABLED
    /// is set to default i.e,
    /// true. If you dont do this explicitley (either by calling this function or by using second param
    /// in Constructor
    ...
    public:
        ...
        /// all the necessary
        /// new line characters are provided. You may define '_STOP_ON_FIRSTELPP_ASSERTION' to make sure
        /// you
        ...
        static bool parseLine(std::string *line, std::string *currConfigStr, std::string *currLevelStr,
            ...
            *line = base::utils::Str::trim(*line);
                ...
                if (quotesStart != std::string::npos && quotesEnd != std::string::npos) {
                    ...
                    ELPP_ASSERT((quotesStart < quotesEnd),
                                "Configuration error - No ending quote found in [" << currConfigStr
                                                                                   << "]");
                    ELPP_ASSERT((quotesStart + 1 != quotesEnd), "Empty configuration value for ["
                                                                    << currConfigStr << "]");
            ...
            ELPP_ASSERT(*currLevel != Level::Unknown, "Unrecognized severity level [" << *currLevelStr
                                                                                      << "]");
            ELPP_ASSERT(currConfig != ConfigurationType::Unknown, "Unrecognized configuration ["
                                                                      << *currConfigStr << "]");
...
///
/// @detail el::Configurations have string based values. This is whats used internally in order to read
/// correct configurations.
/// This is to perform faster while writing logs using correct configurations.
///
/// This is thread safe and final class containing non-virtual destructor (means nothing should inherit
/// this class)
...
public:
    ...
    inline const base::MillisecondsWidth &millisecondsWidth(Level level = Level::Global) {
        return getConfigByRef<base::MillisecondsWidth>(level, &m_millisecondsWidthMap,
                                                       "millisecondsWidth");
    }
...
private:
    ...
    inline Conf_T getConfigByVal(Level level, const std::map<Level, Conf_T> *confMap,
                                 const char *confName) {
        base::threading::ScopedLock scopedLock(lock());
    ...
    template <typename Conf_T>
    inline Conf_T &unsafeGetConfigByRef(Level level, std::map<Level, Conf_T> *confMap,
                                        const char *confName) {
        ELPP_UNUSED(confName);
    ...
    void build(Configurations *configurations) {
        ...
        for (Configurations::const_iterator it = configurations->begin(); it != configurations->end();
            ...
            if (conf->configurationType() == ConfigurationType::Enabled) {
                ...
                setValue(conf->level(),
                         base::LogFormat(conf->level(), base::type::string_t(conf->value().begin(),
                                                                             conf->value().end())),
                         &m_logFormatMap);
        ...
        // As mentioned early, we will now set filename configuration in separate loop to deal with
        // non-existent files
        for (Configurations::const_iterator it = configurations->begin(); it != configurations->end();
             ++it) {
            Configuration *conf = *it;
    ...
    unsigned long getULong(std::string confVal) {
        ...
        valid = !confVal.empty() &&
                std::find_if(confVal.begin(), confVal.end(),
    ...
    std::string resolveFilename(const std::string &filename) {
        ...
        if ((dateIndex = resultingFilename.find(dateTimeFormatSpecifierStr.c_str())) !=
            std::string::npos) {
            while (dateIndex > 0 &&
                   resultingFilename[dateIndex - 1] == base::consts::kFormatSpecifierChar) {
            ...
            if (dateIndex != std::string::npos) {
                ...
                base::utils::Str::replaceAll(
                    now, '/', '-'); // Replace path element since we are dealing with filename
    ...
    void insertFile(Level level, const std::string &fullFilename) {
        ...
        if (resolvedFilename.empty()) {
            std::cerr << "Could not load empty file for logging, please re-check your configurations "
                         "for level [" << LevelHelper::convertToString(level) << "]";
        }
        ...
        auto create = [&](Level level) {
            ...
            if (fs == nullptr) {
                ...
                ELPP_INTERNAL_ERROR("Setting [TO_FILE] of [" << LevelHelper::convertToString(level)
                                                             << "] to FALSE",
                                    false);
...
public:
    Logger(const std::string &id, base::LogStreamsReferenceMap *logStreamsReference)
        : m_id(id), m_typedConfigurations(nullptr), m_parentApplicationName(std::string()),
...
public:
    LogMessage(Level level, const std::string &file, unsigned long int line, const std::string &func,
        ...
        : m_level(level), m_file(file), m_line(line), m_func(func), m_verboseLevel(verboseLevel),
          m_logger(logger), m_message(std::move(logger->stream().str())) {
    }
    inline Level level(void) const {
        return m_level;
    }
...
public:
    ...
    explicit Storage(const LogBuilderPtr &defaultLogBuilder)
        :
          ...
          m_registeredLoggers(new base::RegisteredLoggers(defaultLogBuilder)), m_flags(0x0),
    ...
    template <typename T> inline T *performanceTrackingCallback(const std::string &id) {
        return callback<T, base::type::PerformanceTrackingCallbackPtr>(id,
...
// default log file
        ...
        if (m_commandLineArgs.hasParamWithValue(base::consts::kDefaultLogFileParam)) {
            Configurations c;
            c.setGlobally(ConfigurationType::Filename, std::string(m_commandLineArgs.getParamValue(
...
protected:
    void handle(const LogDispatchData *data) {
        m_data = data;
        dispatch(std::move(m_data->logMessage()->logger()->logBuilder()->build(
                ...
                if (fs != nullptr) {
                    ...
                    ELPP_INTERNAL_ERROR(
                        "Log file for ["
                            ...
                            << "has not been configured but [TO_FILE] is configured to TRUE. [Logger "
                               "ID: " << m_data->logMessage()->logger()->id() << "]",
                        false);
                }
            }
...
public:
    LogDispatcher(bool proceed, LogMessage &&logMessage, base::DispatchAction dispatchAction)
        : m_proceed(proceed), m_logMessage(std::move(logMessage)),
...
///
/// @detail There is workaround needed to loop through some stl containers. In order to do that, we need
/// iterable containers
...
#define ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG(temp)                                                      \
    ...
    template <std::size_t Size> inline MessageBuilder &operator<<(const std::bitset<Size> &bitset_) {
        m_logger->stream() << ELPP_LITERAL("[");
    ...
    template <typename K, typename V> inline MessageBuilder &operator<<(const QMap<K, V> &map_) {
        m_logger->stream() << ELPP_LITERAL("[");
    ...
    template <typename K, typename V> inline MessageBuilder &operator<<(const QHash<K, V> &hash_) {
        m_logger->stream() << ELPP_LITERAL("[");
...
///
/// @detail This macro expands to write an ostream& operator<< for container. This container is expected
/// to
...
/// @param ElementInstance Instance of element to be fed out. Insance name is "elem". See WXELPP_ENABLED
/// macro
...
protected:
    ...
    void triggerDispatch(void) {
        if (m_proceed) {
            base::LogDispatcher(m_proceed,
                                LogMessage(m_level, m_file, m_line, m_func, m_verboseLevel, m_logger),
...
void Logger::log_(Level level, int vlevel, const char *s, const T &value, const Args &... args) {
    ...
    ELPP_INTERNAL_ERROR(
        "Too many arguments provided. Unable to handle. Please provide more format specifiers", false);
}
template <typename T> inline void Logger::log_(Level level, int vlevel, const T &log) {
    if (level == Level::Verbose) {
...
public:
    PerformanceTracker(const std::string &blockName,
        ...
        : m_blockName(blockName), m_timestampUnit(timestampUnit), m_loggerId(loggerId),
          m_scopedLog(scopedLog), m_level(level), m_hasChecked(false), m_lastCheckpointId(std::string()),
          m_enabled(false) {
...
protected:
    void handle(const PerformanceTrackingData *data) {
        ...
        el::base::Writer(m_data->performanceTracker()->level(), m_data->file(), m_data->line(),
                         m_data->func()).construct(1, m_data->loggerId().c_str())
            << ss.str();
    }
    ...
    public:
        ...
        friend std::ostream &operator<<(std::ostream &ss, const StackTraceEntry &si) {
            ss << "[" << si.m_index << "] " << si.m_location << (si.m_demangled.empty() ? "" : ":")
               << si.m_demangled << (si.m_hex.empty() ? "" : "+") << si.m_hex << si.m_addr;
            return ss;
        }
...
public:
    ...
    /// @brief Installs pre rollout callback, this callback is triggered when log file is about to be
    /// rolled out
...
public:
    ...
    static inline Logger *reconfigureLogger(const std::string &identity,
    ...
    /// @brief Sets default configurations. This configuration is used for future (and conditionally for
    /// existing) loggers
    ...
    /// configure logger.
    ///         If true is returned that does not mean it has been configured successfully, it only means
    ///         that it
...
#define DCSYSLOG_IF(condition, LEVEL, ...)                                                              \
    C##LEVEL##_IF(el::base::Writer, (ELPP_DEBUG_LOG) && (condition), el::base::DispatchAction::SysLog,  \
                  __VA_ARGS__)
...
namespace base {
namespace utils {
template <typename T> static T *checkNotNull(T *ptr, const char *name, const char *loggers, ...) {
...
#define INITIALIZE_LOGGER                                                                               \
    ELPP_INIT_LOGGER(new el::base::Storage(el::LogBuilderPtr(new el::base::DefaultLogBuilder()),        \
...
#define SHARE_LOGGER(initializedStorage)                                                                \
    namespace el {                                                                                      \
    namespace base {                                                                                    \
...
#if defined(ELPP_UNICODE)
#define START_LOGGER(argc, argv)                                                                        \
    el::Helpers::setArgs(argc, argv);                                                                   \
    std::locale::global(std::locale(""))
#else
#define START_LOGGER(argc, argv) el::Helpers::setArgs(argc, argv)
#endif // defined(ELPP_UNICODE)
#endif // LOGGER_H
